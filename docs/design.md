...
```
┌─ 视图 ─────────────────────────────────────────────┐
│ [搜索框  ⚲ 搜索轨迹…]          图例（默认展开，可折叠）│  ← 顶栏
│ 图例展开时：💬你  🤖AI  🧠思考  🔧工具  ✓/✗结果  …    │
├────────────────────────────────────────────────────┤
│ ┌─ 第 1 轮 · 你问：给项目加个 README… ── 12s · 模型 · 3.2k token ┐ │
│ │  💬 你：给项目加个 README                                     │ │
│ │  🧠 AI 思考（约 40 字）…                          [展开]       │ │
│ │  🔧 读取文件 docs/design.md · 一句话说明           [详情]       │ │
│ │  ✓ 读取文件 完成 · 预览第一行…                     [详情]       │ │
│ │  🤖 AI 回复：好的，我来…（截断）                   [展开]       │ │
│ └───────────────────────────────────────────────────────────┘ │
│ ┌─ 第 2 轮 · 你问：… ── 正在运行 ── [正在思考] ┐                 │
...
│ └──────────────────────────────────────────────┘               │
│            [加载更早记录]  /  加载中…（禁用态）                    │
│ （空状态：还没有轨迹。给 AI 发一条消息，这里会记录完整的互动过程。）│
└────────────────────────────────────────────────────────────────┘
```
...
| `assistant` | 🤖 步骤行；`blocks` 分类：`text`→回复、`reasoning`→🧠思考、`tool-call`→🔧工具调用行、`image`→🖼图片、`other`→其他内容；meta：`provenance.model`、`usage`、`interrupted`→「已停止」 | 思考默认折叠；回复长文本截断 |
| `tool-result` | ✓/✗ + 中文工具名（`call.name`）+ 结果预览（首个 text 块）；`isError`→✗+error 摘要；耗时 `time - callTime` | 原始内容进「详情」 |
| `context` | ℹ️ 系统注入了上下文信息（`provenance` 可读字段并入说明） | — |
| `compaction` | 🧹 历史压缩：把之前的对话压缩成摘要（省 token）；`shadowedItemCount`/`shadowedTokenCount` 入详情 | summary 进「详情」 |
| `steering` | ✍️ 你补充说：文本 | 同上截断 |
| `turn-error` | ❌ 本轮出错：`message`（`code` 附后） | — |
| `turn-max-tokens` | ⏹ 回复达到长度上限被截断 | — |
| `model-retry` | 🔁 模型请求失败，已自动重试（`retryState` 入详情） | — |
| `command` / `command-input` | ⌘ 执行命令 `/name` / 命令输入 | 参数入「详情」 |
| `workflow-run` | ⚙️ 工作流运行 | 入「详情」 |
| `unknown` | · 其他事件 | — |
...
- 图例默认展开（新用户），右上角可折叠，折叠后记忆在组件本地状态。
- 进行中回合始终贴在卡片流末尾；向上滚动不打断（列表自然滚动，不做虚拟化）。
