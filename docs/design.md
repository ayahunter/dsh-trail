...
┌─ 视图 ─────────────────────────────────────────────┐
│ [搜索轨迹…]                                [隐藏筛选]│  ← 顶栏
│ [全部] [💬你] [🤖AI回复] [🧠AI思考] [🛠️工具] [✅成功]… │
├────────────────────────────────────────────────────┤
│ ┌─ 第 1 轮 · 你问：给项目加个 README  8s · gpt-4o · 3.2k token ┐ │
│ │  👤 你：给项目加个 README                                     │ │
│ │  🧠 AI 思考：用户希望为项目添加…（60 字截断）      [展开]        │ │
│ │  🛠️ 读取文件：docs/design.md                        [详情]        │ │
│ │  ✅ 读取文件成功：docs/design.md                    [详情]        │ │
│ │  🤖 AI 回复：好的，我来为这个项目创建…（180 字摘要） [展开]      │ │
│ └───────────────────────────────────────────────────────────┘ │
│ ┌─ 第 2 轮 · 你问：再加部署说明  正在运行 · gpt-4o ┐              │
│ │  🧠 AI 思考：…                         [正在思考] │              │
│ │  🛠️ 读取文件：package.json             [正在运行] │              │
│ │  》 AI 回复：…                           [生成中] │              │
│ └─────────────────────────────────────────────────┘            │
│ ┌─ 第 3 轮 · 你问：补充贡献指南  8s · gpt-4o ───────┐            │
...
| `kind` | 渲染（v2） | 折叠策略 |
|---|---|---|
| `user` | 💬 你：文本（content 的 text 块拼接） | 120 字摘要，长文本可展开 |
| `assistant` | 步骤行；`blocks` 分类：`text`→「🤖 AI 回复：…」、`reasoning`→「🧠 AI 思考：…」、`tool-call`→「🛠️ 中文名：<参数预览>」、`image`→图片、`other`→系统事件；meta：`provenance.model`、`usage`、`interrupted`→「已停止」 | 思考默认折叠（60 字）；回复摘要 180 字；展开态按原换行显示全文 |
| `tool-result` | 「✅ 中文名成功：<参数预览>」；`isError`→「✕ 中文名失败：<错误>」 | 工具说明、原始参数、完整结果、耗时和错误元数据进「详情」 |
...
- 行详情展开状态用组件本地 `Set`（键 `n<seq>` / `b<seq>_<i>`）；回合折叠覆盖用本地 `Map`。切换不改快照、不写全局。
- 「详情」抽屉式行内展开（同一行下方展开原始参数/结果/计时/token），不用弹窗。
- 搜索：按轨迹行匹配摘要及隐藏详情。文本先做 NFKC 和小写归一化，再忽略空白与标点；每个关键词先连续子串匹配，未命中时对两个字符以上的词做字符顺序模糊匹配，多词必须全部命中。结果保持原始时间顺序，不做拼音、编辑距离或相关度排序。
- 类别条默认展开，八类定义集中保存名称、emoji 与搜索别名。初始为「全部」；从全部点击单类会隔离该类，之后可继续叠加或移除，最后一类再次点击恢复全部。选择写入 `dsh-trail.visible-categories.v1`，非法、过期或空数据回退全部。
- 搜索或类别筛选只保留命中行及所属回合，回合标题与 meta 不参与类别过滤；筛选变化清除手动折叠覆盖并自动展开结果。
- 有历史节点时，`partial` / `runningCalls` 合并进最后一个回合卡片；尚无回合时才显示独立进行中卡片。行尾徽标显示「正在思考 / 生成中… / 正在运行 Ns」；耗时随流式帧自然刷新，不额外起定时器、不虚构耗时。
...
- 图标使用 emoji 与 CSS 语义色容器，不增加图标依赖；工具调用使用 🛠️，成功结果使用 ✅；搜索和折叠控制使用 CSS 线条，回合 chevron 通过旋转表达状态。
